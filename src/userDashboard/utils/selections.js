...
function getNonimmigrationStatusByCode(code) {
  return NONIMMIGRATION_STATUS.find(status => status.code === code).name;
}

function getCountryNameByCode(code) {
  return COUNTRIES.find(status => status.code === code).name;
}
...
for(let status of NONIMMIGRATION_STATUS)
  statusSelections.push({name: status.name, value: status.code});

let PHONE_REGION_NUM = [
  {name: "美国(+1)", value: "1"},
  {name: "中国(+86)", value: "86"},
];

export {nationality_selection, statusSelections, PHONE_REGION_NUM, getCountryNameByCode, getNonimmigrationStatusByCode};